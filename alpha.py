...
class Solution:
    ...
    def getrow(self, result):
        ...
        if flag == 8:
            while y < self.nTracks:
                count = count + 1
                if count % (ratio + 1) == 0 and ratio != 0:
        ...
        for i in range(len(self.vCells)):
            ...
            for j in range(len(self.rows)):
                ...
                if temp < disp and self.vCells[i].height == self.rows[j].Rheight and self.nSites - self.rows[j].RX >= self.vCells[i].width:
                    disp = temp
                    rowInsert = j
            self.vCells[i].locX = self.rows[rowInsert].RX
            self.vCells[i].locY = self.rows[rowInsert].RY
            self.rows[rowInsert].RX = self.rows[rowInsert].RX + self.vCells[i].width
        for i in range(len(self.vCells)):
            if i == len(self.vCells) - 1:
                result = result + "c" + str(i) + " " + str(self.vCells[i].locY) + " " + str( self.vCells[i].locX)
            else:
                result = result + "c" + str(i) + " " + str(self.vCells[i].locY) + " " + str( self.vCells[i].locX) + "\n"
        return result
    ...
    def result(self, path):
        ...
        result = self.legalize(result)
        return result

if __name__ == "__main__":
    if len(sys.argv) < 3 or len(sys.argv) > 4:
        print("usage: reduce.py <in_file> <out_file> [unate_reduce_log_file]")
        exit(1)
